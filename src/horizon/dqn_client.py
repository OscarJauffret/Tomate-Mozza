...
class HorizonClient(Client):
    def __init__(self, num, shared_dict) -> None:
        ...
        self.trainer = QTrainer(self.model, self.device, self.hyperparameters["learning_rate"],
        ...
        if not os.path.exists(os.path.join(get_states_path(), Config.Paths.MAP)):
            ...
            self.random_states = [os.path.join(Config.Paths.MAP, state) for state in
                                  os.listdir(os.path.join(get_states_path(), Config.Paths.MAP)) if
                                  state.endswith(".bin")]
    ...
    def load_model(self) -> None:
        if self.model_path.qsize() > 0:
            ...
            if os.path.exists(model_pth):
                ...
                self.memory = PrioritizedReplayBuffer(self.hyperparameters["max_memory"],
    ...
    def determine_done(self, simulation_state: SimStateData) -> torch.Tensor:
        ...
        if self.prev_positions and len(self.prev_positions) == (
                5 * Config.Game.NUMBER_OF_ACTIONS_PER_SECOND) and np.linalg.norm(
                np.array(self.prev_positions[0]) - np.array(
                        self.prev_positions[-1])) < 5:  # If less than 5 meters were travelled in the last 5 seconds
            return torch.tensor(1.0, device=self.device, dtype=torch.float)
    ...
    def remember(self, state, action, reward, next_state, done):
        self.memory.add(
            (state, action, reward, next_state, done))  # No priority, it will take the highest priority by default
    ...
    def on_run_step(self, iface: TMInterface, _time: int) -> None:
        ...
        if _time >= 0 and _time % Config.Game.INTERVAL_BETWEEN_ACTIONS == 0 and self.ready:
            ...
            if total_time * 1000 > Config.Game.INTERVAL_BETWEEN_ACTIONS / Config.Game.GAME_SPEED:
                print(
                    f"Warning: the action took {total_time * 1000:.2f}ms to execute, it should've taken less than {Config.Game.INTERVAL_BETWEEN_ACTIONS / Config.Game.GAME_SPEED:.2f}ms")
