...
# Assume NStepBufferA and NStepBufferB are your two buffer classes
...
class TestNStepNewufferEquivalence(unittest.TestCase):
    def setUp(self):
        self.n_steps = 5
    ...
    def test_equivalence(self):
        for state, action, reward in zip(self.states, self.actions, self.rewards):
            self.bufferOld.add(state, action, reward)
        ...
        # Test action
        self.assertEqual(actionOld, actionNew, "Actions are not equal")
        
        # Test reward cumulée
        self.assertAlmostEqual(R_Old, R_New, places=5, msg="Cumulative rewards differ")
        ...
        while not self.bufferOld.is_empty():
            ...
            self.assertEqual(old[1], new[1], "Actions are not equal")
            self.assertAlmostEqual(old[2], new[2], places=5, msg="Cumulative rewards differ")
        ...
        # Test clear
        self.bufferOld.add(self.states[0], self.actions[0], self.rewards[0])
...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size):
        if self.fill_level < Config.NN.MIN_MEMORY or batch_size > self.fill_level:
            print(self.fill_level, batch_size)
            return None
...
class TestNstepBuffersWithPrioritizedReplayBuffer(unittest.TestCase):
    ...
    def test_equivalence(self):
        ...
        sampled_old = self.prioritized_buffer_old.sample(5)
        sampled_new = self.prioritized_buffer_new.sample(5)
        self.assertIsNotNone(sampled_old, "Old buffer sample is None")
        self.assertIsNotNone(sampled_new, "New buffer sample is None")
        self.assertTrue
